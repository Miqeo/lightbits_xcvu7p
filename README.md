...
This effort is concentrated on reverse engineering of the Lightbits LBASSY0021 Xilinx Virtex Ultrascale+ XCVU7P FLVB2104AAZ FPGA in the form of the PCIe x16 low profile board.

## Programming

The initial assessment of the programming interface was done and yelded the table below. The pinout is similar in nature to the 14-pin interface used in the Xilinx programmators. 

The board was successfully programmed with a mock logic to test wheter the interface if fully functional, as well as whether the board is not locked in some way that would halt its further use. 
...
The response from the board after connecting it to a JTAG probe and using Openocd was correct and was in line with what chip is present on this board as per the [UltraScale Architecture Configuration User Guide (UG570)](https://docs.amd.com/api/khub/maps/ioE1QkNEnAQuonKQY6T3pg/attachments/oSt8I_xWTUZb2SISl9jrPw-ioE1QkNEnAQuonKQY6T3pg/content?download=true&locationValue=reader). 
...
The pin was a type of general I/O, and thus it could be used to verify that the clock source was working. The correct differential pair is made out of pins **AY30**, **BA30** (positive, negative). The final pulse is confirmed to be 100 MHz based on the output of the logic creating a basic clock counter of 1 Hz.

![](/images/programming/probe_clock.png)

*An excerpt from the logic analysis software showing a correct 1 Hz output*

## General purpose I/O derivation

I used a simple universal asynchronous transmitter, sending a unique number for each of the GPIO pins available in this footprint, 702 in total. The pin description effort is ongoing and the current state can be seen in this [constraints file](lightbits.xdc).
...
| 7BDS | ![](/images/ics/7DBS.JPG) | PCA9306DCUX | PCA9306 Dual Bidirectional I2C Bus and SMBus Voltage-Level Translator |
...
Reach out to me if you have any questions regarding this work or if you have some information helpful in reverse engineering of this hardware. 